...
const ExpiredPreApprovals: React.FC = () => {
  const [preApprovals, setExpiredApprovals] = useState<PreApproval[]>([]);
  ...
  const fetchPreApprovals = async () => {
    startLoading();
    try {
      const response = await visitorsService.getPaginatedPreApprovalsStatusExpired(currentPage, pageSize);
      setExpiredApprovals(response.data);
  ...
  return (
    <div>
      <Title level={4}>Expired Pre-Approvals</Title>
      <Input.Search
...
export default ExpiredPreApprovals;
